...
public class ScheduleExamples {
    ...
    public static void scheduleViaEventLoop() {
        ...
        // 创建定时任务
        io.netty.util.concurrent.ScheduledFuture<?> future = eventLoop.schedule(
    ...
    public static void scheduleFixedViaEventLoop() {
        Channel ch = CHANNEL_FROM_SOMEWHERE; // get reference from somewhere
        EventLoop eventLoop = ch.eventLoop();
        io.netty.util.concurrent.ScheduledFuture<?> scheduledFuture = eventLoop.scheduleAtFixedRate(
                // 创建一个 Runnable，以供调度稍后执行
                new Runnable() {
                    @Override
                    public void run() {
